...
public class UcManageAccessCodeImpl extends AbstractAccessCodeUc implements UcManageAccessCode {
	...
	public void deleteAccessCode(long accessCodeId) {
		...
		/**
		 * Using the method getQueuemanagement() gives access to the methods that were
		 * created earlier in the usecasemanage(inside the queue component). This is
		 * done so each component takes care of its own modifications.
		 */
	...
	public AccessCodeEto saveAccessCode(AccessCodeEto accessCodeEto) {
		 ...
		 * method findAccessCodeEtos() that will call then implemention of the method
		 * inside (UcFindAccessCodeImpl) through the interface. This allows us to use
		 * the {@link UcFindAccessCodeImpl}
		 */
		List<AccessCodeEto> accessCodeEtosInQueue = getAccesscodemanagement()
				.findAccessCodeEtos(accessCodeSearchCriteriaTo).getContent();
